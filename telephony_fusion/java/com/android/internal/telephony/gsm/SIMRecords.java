...
public final class SIMRecords extends UiccApplicationRecords {
    ...
    public void handleMessage(Message msg) {
        ...
        //TODO: Fusion - we probably don't need this. Verify.
        ...
        try { switch (msg.what) {
            ...
            case EVENT_GET_SST_DONE:
                ...
                boolean doUpdateSpnDisplay = handleSstData(data);
                if (doUpdateSpnDisplay) {
    ...
    private void handleFileUpdate(int efid) {
        switch(efid) {
            ...
            case IccConstants.EF_SST:
                ...
                mFh.loadEFTransparent(IccConstants.EF_SST,
     ...
     */
    boolean checkAndFetchEonsName(String regOperator, GsmCellLocation loc) {

        boolean needsSpnDisplayUpdate = false;
        ...
        if (sstPlmnOplValue == PNN_OPL_ENABLED) {
            ...
            needsSpnDisplayUpdate = fetchEonsName(regOperator, loc);
            ...
            needsSpnDisplayUpdate = fetchPnnFirstRecord(regOperator);
        } else {
            Log.e(LOG_TAG,"EONS: Invalid sstPlmnOplValue " + sstPlmnOplValue);
        }

        return needsSpnDisplayUpdate;
    }
     ...
     */
    public boolean fetchEonsName(String regOperator, GsmCellLocation loc) {
        short oplDataPnnNum;
        int oplDataLac1;
        int oplDataLac2;
        int count = 0;
     ...
     */
    boolean fetchPnnFirstRecord(String regOperator) {

        boolean doUpdateSpnDisplay = false;
        ...
        if ((getSIMOperatorNumeric() != null) &&
            ...
            doUpdateSpnDisplay = getNameFromPnnRecord(1);
        } else {
            pnnDataPresent = false;
            doUpdateSpnDisplay = true;
        }

        return doUpdateSpnDisplay;
    }

    /**
     * Parse EF_SST data.
     */
    boolean handleSstData(byte[] data) {

        boolean needsSpnDisplayUpdate = false;

        try {
            if (mParentCard.isApplicationOnIcc(AppType.APPTYPE_SIM)) {
                //2G Sim.
            ...
            } else if (mParentCard.isApplicationOnIcc(AppType.APPTYPE_USIM)) {
                //3G Sim.
    ...
    boolean useMEName() {
        boolean needsSpnDisplayUpdate = true;
     ...
     */
    boolean getNameFromPnnRecord(int record) {
        ...
        for (i = 0;i < usedCspGroups;i++) {
             if (data[2 * i] == valueAddedServicesGroup) {
                 ...
                 if ((data[(2 * i) + 1] & 0x80) == 0x80) {
                     ...
                     mNetworkSelectionModeAutomaticRegistrants.notifyResult(obtainMessage(EVENT_AUTO_SELECT_DONE));
                 }
                 return;
             }
        }
