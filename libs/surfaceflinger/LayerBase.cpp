...
GLuint LayerBase::createTexture(PixelFormat pixelFormat) const
{
    ...
    if((pixelFormat == HAL_PIXEL_FORMAT_YCbCr_420_SP_TILED) ||
       (pixelFormat == HAL_PIXEL_FORMAT_YCrCb_420_SP_ADRENO)) {
        ...
        const sp<GraphicBuffer>& buffer, Texture* texture)
{
    ...
    if (texture->image != EGL_NO_IMAGE_KHR) {
        if ((buffer->getPixelFormat() == HAL_PIXEL_FORMAT_YCbCr_420_SP_TILED) ||
            (buffer->getPixelFormat() == HAL_PIXEL_FORMAT_YCrCb_420_SP_ADRENO)) {
...
sp<OverlayRef> LayerBaseClient::Surface::createOverlay(
        uint32_t w, uint32_t h, int32_t format, int32_t orientation, int format3D)
{
    return NULL;
};
