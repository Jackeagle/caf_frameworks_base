        ...
        case QOMX_COLOR_FormatYUV420PackedSemiPlanar64x32Tile2m8ka:
            {
                ...
                if (lib == NULL) {
                    LOGE("Could not open mm-color-convertor library: %s", dlerror());
                    return;
                }

                convertFn convertNV12Tile = (convertFn)dlsym(lib, "_Z7convertN7android18ColorConvertParamsES0_Ph");
                if (convertNV12Tile == NULL) {
                    LOGE("Could not find convert symbol: %s", dlerror());
                    return;
                }

                uint8_t *adjustedClip = initClip();
                struct ColorConvertParams src;
                src.width = srcSkip;
                src.height = height;
                src.cropWidth = width;
                src.cropHeight = height;
                src.cropLeft = 0;
                src.cropRight = width;
                src.cropTop = 0;
                src.cropBottom = height;
                src.data = srcBits;
                src.colorFormat = YCbCr420Tile;
                src.flags = 0;

                struct ColorConvertParams dst;
                dst.width = dstSkip;
                dst.height = height;
                dst.cropWidth = width;
                ...
                dst.cropRight = width;
                dst.cropTop = 0;
                dst.cropBottom = height;
                dst.data = dstBits;
                dst.colorFormat = RGB565;
                dst.flags = 0;

                int retval = convertNV12Tile(src, dst, adjustedClip);
                CHECK(!retval);
                dlclose(lib);
                break;
            }
