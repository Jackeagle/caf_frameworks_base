...
#define PAUSE_WAIT_TIME 200000000 //200 ms
...
status_t LPAPlayer::start(bool sourceAlreadyStarted) {
    ...
    mNumOutputChannels = 2;  // Always hard code Output Channels to 2^M
    if (!bIsA2DPEnabled) {
    ...
    param_set_int(params, SNDRV_PCM_HW_PARAM_FRAME_BITS,
                mNumOutputChannels - 1 ? 32 : 16);
    param_set_int(params, SNDRV_PCM_HW_PARAM_CHANNELS, mNumOutputChannels);
        ...
        if (a2dpDisconnectPause) {
            ...
            if (!bIsA2DPEnabled) {
                ...
                if (local_handle->start != 1) {
                    pause_cv.waitRelative(pause_mutex, PAUSE_WAIT_TIME); //timedwait for 200 ms
                }
                if (ioctl(local_handle->fd, SNDRV_PCM_IOCTL_PAUSE,1) < 0) {
                    LOGE("Audio Pause failed");
                }
    ...
    while (1) {
        ...
        //Queue up the buffers for writing either for A2DP or LPA Driver
            ...
            if( mNumInputChannels == 1) {
                buf.bytesToWrite = fillBuffer(buf.localBuf, MEM_BUFFER_SIZE/2);
    ...
    while (1) {
        ...
        if (pfd[1].revents & POLLIN) {
            ...
            if (u == SIGNAL_EVENT_THREAD) {
                ...
                timeout = 1000 * tempbuf.bytesToWrite / (mNumOutputChannels * PCM_FORMAT * mSampleRate);
...
int64_t LPAPlayer::getTimeStamp(A2DPState state) {
    ...
    case A2DP_DISCONNECT:
        timestamp = (mNumA2DPBytesPlayed * 1000000)
                    /(2 * mNumOutputChannels * mSampleRate);
        break;
    case A2DP_DISABLED:
    case A2DP_CONNECT: {
        struct pcm * local_handle = (struct pcm *)handle;
