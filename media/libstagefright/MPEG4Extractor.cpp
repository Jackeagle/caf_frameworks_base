...
struct MPEG4DataSource : public DataSource {
    ...
    status_t setCachedRange(sfoff_t offset, size_t size);
...
ssize_t MPEG4DataSource::readAt(sfoff_t offset, void *data, size_t size) {
...
status_t MPEG4DataSource::getSize(sfoff_t *size) {
...
status_t MPEG4DataSource::setCachedRange(sfoff_t offset, size_t size) {
    Mutex::Autolock autoLock(mLock);
...
status_t MPEG4Extractor::parseChunk(sfoff_t *offset, int depth) {
    uint32_t hdr[2];
    ...
    sfoff_t chunk_data_size = *offset + chunk_size - data_offset;

    if (chunk_type != FOURCC('c', 'p', 'r', 't')
            && mPath.size() == 5 && underMetaDataPath(mPath)) {
        sfoff_t stop_offset = *offset + chunk_size;
        *offset = data_offset;
...
status_t MPEG4Extractor::parseMetaData(sfoff_t offset, size_t size) {
    if (size < 4) {
        return ERROR_MALFORMED;
    }
