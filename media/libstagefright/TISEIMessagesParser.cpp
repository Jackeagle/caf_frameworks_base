...
/* Subclause D.1.25 */
int16_t frame_packing_arrangement(mp4StreamType *psBits, S3D_params &mS3Dparams)
{
    // SEI message detected, default configuration
    mS3Dparams.active = true;
    mS3Dparams.mode = S3D_MODE_ON;
...
{
    int16_t status = 0;
    int32_t last_payload_type_byte;
    int32_t last_payload_size_byte;
  ...
  *     We are only supporting stereo_video_info and frame_packing_arrangement
  *     SEI messages, hence this is not necessary at this layer
  ----------------------------------------------------------------------------*/
    //do
    //{
