...
static int32_t drm_saveFLRights(uint8_t* key, uint8_t* cid)
{
    ...
    if (drm_readFromUidTxt(cid, &id, GET_ID) == FALSE) {
        ...
        if (FALSE == drm_appendRightsInfo(&rights)) {
            DRMV1_D("-------------------------------FAILED TO APPENDRIGHTS INFO---------\n");
            return -1;
        }
    }
...
static int32_t drm_formatDCFHeader(uint8_t* pHeader, T_DRM_DM_Info* dmInfo)
{
    ...
    DRMV1_D("enter drm_generateFLDCFHeader with dmInfo %p.\n", dmInfo);
...
static void drm_addPaddingBytes(uint8_t* pRawBuf, int32_t remainBytes)
{
    ...
    DRMV1_D("remainBytes is %d\n", remainBytes);

    DRMV1_D("rfc2630Pad is %d\n", rfc2630Pad);
...
int32_t drm_generateDCFHeader(T_DRM_Enc_Context* ctx)
{
    ...
    if (write(ctx->dcfHandle, header, totalSize) != totalSize) {
        DRMV1_E("write DCF header error.\n");
        return -1;
    }
    return 0;
}
...
int32_t drm_encContent(T_DRM_Enc_Context* ctx, int32_t handle)
{
    ...
    DRMV1_D ("remain cipher length [%d]", remainCipherLen);

    //reposition to content start
    lseek (handle, ctx->dmInfo->contentOffset, SEEK_SET);
    do {
        ...
        if (toReadBytes > 0) {
            ...
            if (cipherLen != toReadBytes) {
                DRMV1_E("read cipher data error from file, length [%d], errinfo [%s]", cipherLen, strerror(errno));
                return -1 ;
            }
        ...
        if (remainCipherLen == 0) {
            DRMV1_D ("Last block length [%d]", cipherLen);
        ...
        if (write(ctx->dcfHandle, encryptedBlock, DRM_ONE_AES_BLOCK_LEN) != DRM_ONE_AES_BLOCK_LEN) {
            DRMV1_E("write encrypted buf to file error.\n");
            return -1;
        }
    } while (!padded);

    DRMV1_D ("out of drm_encContent");
