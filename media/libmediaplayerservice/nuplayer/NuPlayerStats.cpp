...
void NuPlayerStats::logStatistics() {
    if(mStatistics) {
        Mutex::Autolock autoLock(mStatsLock);
        LOGW("=====================================================");
        LOGW("Mime Type: %s",mMIME);
        LOGW("Number of frames dropped: %lld",mNumVideoFramesDropped);
        LOGW("Number of frames rendered: %llu",mTotalFrames);
        LOGW("=====================================================");
    }
}
...
void NuPlayerStats::logPause(int64_t positionUs) {
    if(mStatistics) {
        LOGW("=====================================================");
        LOGW("Pause position: %lld ms",positionUs/1000);
        LOGW("=====================================================");
    }
}
...
void NuPlayerStats::logSeek(int64_t seekTimeUs) {
    if(mStatistics) {
        Mutex::Autolock autoLock(mStatsLock);
        LOGW("=====================================================");
        LOGW("Seek position: %lld ms",seekTimeUs/1000);
...
void NuPlayerStats::logFpsSummary() {
    if (mStatistics) {
        ...
        logSyncLoss();
        {
            Mutex::Autolock autoLock(mStatsLock);
            LOGW("=========================================================");
            LOGW("Average Frames Per Second: %.4f", mFPSSumUs/((double)mStatisticsFrames));
            LOGW("Total Frames / Total Time: %.4f", ((double)(mTotalFrames-1)*1E6)/((double)mTotalTime));
            LOGW("========================================================");
        }
    }
}
