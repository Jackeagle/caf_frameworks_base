...
public class ObserverNodeTest extends AndroidTestCase {   
    ...
    public void testUri() {
        ...
        // special case
        root.addObserver(uris[0], new TestObserver().getContentObserver(), false, root);
        for(int i = 1; i < uris.length; i++) {
            root.addObserver(uris[i], new TestObserver().getContentObserver(), true, root);
        }
    ...
    public void testUriNotNotify() {
        ...
        for(int i = 0; i < uris.length; i++) {
            root.addObserver(uris[i], new TestObserver().getContentObserver(), false, root);
        }
