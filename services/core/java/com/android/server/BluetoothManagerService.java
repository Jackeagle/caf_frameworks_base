...
class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    private final class Led {
        ...
        private final Light BluetoothLight;

        public Led(Context context, LightsManager lights) {
            BluetoothLight = lights.getLight(LightsManager.LIGHT_ID_BLUETOOTH);
        }

        public void updateLights(boolean isOn) {
            if (isOn)
                BluetoothLight.setBrightness(0xFF);
            else
                BluetoothLight.setBrightness(0);
        }
    }
