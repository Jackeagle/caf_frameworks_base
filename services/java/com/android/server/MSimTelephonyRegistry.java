...
class MSimTelephonyRegistry extends ITelephonyRegistryMSim.Stub {
    ...
    MSimTelephonyRegistry(Context context) {
        ...
        mCellInfo = new CellInfo[numPhones];
            ...
            synchronized (mRecords) {
                ...
                if (notifyNow) {
                    ...
                    if ((events & PhoneStateListener.LISTEN_CELL_INFO) != 0) {
                        try {
                            r.callback.onCellInfoChanged(new CellInfo(mCellInfo[subscription]));
                        } catch (RemoteException ex) {
                            remove(r.binder);
                        }
                    }
    ...
    public void notifyCellInfo(CellInfo cellInfo, int subscription) {
        if (!checkNotifyPermission("notifyCellInfo()")) {
            return;
        }

        synchronized (mRecords) {
            mCellInfo[subscription] = cellInfo;
            for (Record r : mRecords) {
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        synchronized (mRecords) {
            ...
            for (int i = 0; i < MSimTelephonyManager.getDefault().getPhoneCount(); i++) {
                ...
                pw.println("  mCellInfo=" + mCellInfo[i]);
            }
