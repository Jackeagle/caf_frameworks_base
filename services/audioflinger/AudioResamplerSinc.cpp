    ...
    mState(0)
{
    ...
    //Open the dll to get the coefficients for QCOM_QUALITY
    if (quality == QCOM_QUALITY ) {
        mResampleCoeffLib = dlopen("libmm-audio-resampler.so", RTLD_NOW);
        LOGV("Open libmm-audio-resampler library = %p",mResampleCoeffLib);
        if (mResampleCoeffLib == NULL) {
            LOGE("Could not open mm-audio-resampler library: %s", dlerror());
            return;
        }
        ...
        if (!mReadResampleCoefficients  || !mReadResampleFirNumCoeff || !mReadResampleFirLerpIntBits) {
            ...
            mResampleCoeffLib = NULL;
            LOGE("Could not find convert symbol: %s", dlerror());
            return;
        }
        // we have 16 coefs samples per zero-crossing
        coefsBits = mReadResampleFirLerpIntBits();
        LOGV("coefsBits = %d",coefsBits);
        cShift = kNumPhaseBits - coefsBits;
        cMask  = ((1<<coefsBits)-1) << cShift;
        pShift = kNumPhaseBits - coefsBits - pLerpBits;
        pMask  = ((1<<pLerpBits)-1) << pShift;
        ...
        LOGV("halfNumCoefs = %d",halfNumCoefs);
    }
}
...
AudioResamplerSinc::~AudioResamplerSinc()
{
    if(mResampleCoeffLib) {
        LOGV("close the libmm-audio-resampler library");
        dlclose(mResampleCoeffLib);
        mResampleCoeffLib = NULL;
        mReadResampleCoefficients = NULL;
        mReadResampleFirNumCoeff = NULL;
