...
public final class CallManager {
    ...
    public void acceptCall(Call ringingCall) throws CallStateException {
        ...
        Context context = getContext();
        if (context == null) {
            Log.d(LOG_TAG, "Speedup Audio Path fix: Context is null");
        } else {
            Log.d(LOG_TAG, "Speedup Audio Path fix");
            AudioManager audioManager = (AudioManager)
                    context.getSystemService(Context.AUDIO_SERVICE);
            int currMode = audioManager.getMode();
            if ((currMode != AudioManager.MODE_IN_CALL) && !(ringingPhone instanceof SipPhone)) {
                Log.d(LOG_TAG, "Speedup Audio Path fix: setAudioMode Setting audio mode from "
