...
** Copyright 2010, Texas Instruments - created for HID
**
...
#ifdef HAVE_BLUETOOTH
static jmethodID method_onInputPropertyChanged;
...
static jboolean connectInputNative(JNIEnv *env, jobject object, jstring path) {
    ...
    if (nat) {
        const char *c_path = env->GetStringUTFChars(path, NULL);
		LOGV("DBUS call connectInputNative \n");
...
static jboolean disconnectInputNative(JNIEnv *env, jobject object,
    ...
    if (nat) {
        const char *c_path = env->GetStringUTFChars(path, NULL);
		LOGV("DBUS call disconnectInputNative \n");
        bool ret = dbus_func_args_async(env, nat->conn, -1, NULL, NULL, nat,
...
#ifdef HAVE_BLUETOOTH
    method_onInputPropertyChanged = env->GetMethodID(clazz, "onInputPropertyChanged",
