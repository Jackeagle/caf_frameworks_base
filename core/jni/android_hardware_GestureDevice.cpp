...
sp<GestureDevice> get_native_gesture_device(JNIEnv *env, jobject thiz, JNIGestureDeviceContext** pContext)
{
    ...
    ALOGV("get_native_gesture_device: context=%p, vision=%p", context, gesture.get());
    if (gesture == 0) {
...
{
    if (NULL == gs_results) {
        ALOGE("postResult: gesture result is NULL");
        return;
    }
    ...
    obj = (jobjectArray) env->NewObjectArray(gs_results->number_of_events,
                                             mGestureResultClass, NULL);
    if (NULL == obj) {
        ALOGE("Couldn't allocate Gesture EventPayload array");
        return;
    }

    ALOGI("postResult: Total gs results %d", gs_results->number_of_events);
    for (int i = 0; i < gs_results->number_of_events; i++) {
        ...
        ALOGI("Result %d: version(%d), type(%d), subtype(%d), id(%d), timestamp(%llu), confidence(%f), velocity(%f)",
        ...
        ALOGI("Result %d: location points (%d)", i, gs_results->events[i].location.num_of_points);
...
void JNIGestureDeviceContext::notify(int32_t msgType, int32_t ext1, int32_t ext2)
{
    ...
    Mutex::Autolock _l(mLock);
    if (mGestureDeviceJObjectWeak == NULL) {
        ALOGW("callback on dead vision device object");
        return;
    }
    JNIEnv *env = AndroidRuntime::getJNIEnv();
...
void JNIGestureDeviceContext::postData(gesture_result_t* gs_results)
{
    ...
    JNIEnv *env = AndroidRuntime::getJNIEnv();
    if (mGestureDeviceJObjectWeak == NULL) {
        ALOGW("callback on dead gesture device object");
        return;
    }
...
static void android_hardware_gesturedev_GestureDevice_native_release(JNIEnv *env, jobject thiz)
{
    ...
    if (context != NULL) {
        gesture = context->getGestureDevice();
        context->release();
        ALOGV("native_release: context=%p gesture=%p", context, gesture.get());
...
static jstring android_hardware_gesturedev_GestureDevice_native_getParameters(JNIEnv *env, jobject thiz)
{
    ...
    ALOGE("JNI getParameters: %s", param.string());
...
int register_android_hardware_GestureDevice(JNIEnv *env)
{
    ...
    if (fields.post_event == NULL) {
        ALOGE("Can't find android/hardware/gesturedev/GestureDevice.postEventFromNative");
        return -1;
    }
    ...
    if (fields.evt_constructor == NULL) {
        ALOGE("Can't find android/hardware/gesturedev/GestureResult.GestureResult()");
        return -1;
    }
    ...
    if (fields.vector_constructor == NULL) {
        ALOGE("Can't find android/hardware/gesturedev/GestureResult.GSVector()");
        return -1;
    }
